...
<div class="weapons index">
	...
	<table class="table table-striped table-bordered table-listings" data-sort-url="<?php echo Router::url(array('controller' => 'weapons', 'action' => 'save_order')); ?>">
		...
		<tbody>
			...
			<tr id="weapons-<?php echo $weapon['Weapon']['id']; ?>" data-id="<?php echo $weapon['Weapon']['id']; ?>">
				...
				<td><?php echo $this->Html->link($weapon['Races']['name'], array('controller' => 'races', 'action' => 'view', $weapon['Races']['id'])); ?></td>
				<td><?php echo h($this->Time->timeAgoInWords($weapon['Weapon']['created'])); ?></td>
				<td><?php echo h($this->Time->timeAgoInWords($weapon['Weapon']['modified'])); ?></td>
				<td class="actions">
    ...
    <?php } else { ?>
        <p class='clearfix'>No <?php echo __('Weapons');?> available, why not <?php echo $this->Html->link(__('create one'), array('action' => 'add')); ?>
</p>
    <?php } ?>
