<div class="races index">
    <div class="row">
        <h2 class="span6"><?php echo __('Races');?></h2>
	...
	<table class="table table-striped table-bordered table-listings" data-sort-url="<?php echo Router::url(array('controller' => 'races', 'action' => 'save_order')); ?>">
		...
		<tbody>
			...
			<tr id="races-<?php echo $race['Race']['id']; ?>" data-id="<?php echo $race['Race']['id']; ?>">
								...
								<td><?php echo $this->Html->link($race['RaceTypes']['id'], array('controller' => 'race_types', 'action' => 'view', $race['RaceTypes']['id'])); ?></td>
								<td><?php echo h($race['Race']['name']); ?></td>
								<td><?php echo h($race['Race']['created']); ?></td>
								<td><?php echo h($race['Race']['modified']); ?></td>
								<td class="actions">
					<?php echo $this->Html->link(__('View'), array('action' => 'view', $race['Race']['id']), array('class' => 'icon icon-eye')); ?>
					<?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $race['Race']['id']), array('class' => 'icon icon-edit')); ?>
					<?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $race['Race']['id']), array('class' => 'icon icon-delete'), __('Are you sure you want to delete this record?')); ?>
				</td>
			</tr>
			<?php } ?>
		</tbody>
